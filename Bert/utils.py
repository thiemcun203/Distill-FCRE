...
class Moment:
    ...
    def init_moment(self, encoder, dataset, is_memory=False):
        ...
        datalen = len(dataset)
        if not is_memory:
            self.features = torch.zeros(datalen, self.config.encoder_output_size).to(torch.bfloat16 if self.config.dtype == 'bfloat16' else torch.float32)
            self.features_des = torch.zeros(datalen, self.config.encoder_output_size).to(torch.bfloat16 if self.config.dtype == 'bfloat16' else torch.float32)
            ...
            self.mem_features = torch.zeros(datalen, self.config.encoder_output_size).to(torch.bfloat16 if self.config.dtype == 'bfloat16' else torch.float32)
            self.mem_features_des = torch.zeros(datalen, self.config.encoder_output_size).to(torch.bfloat16 if self.config.dtype == 'bfloat16' else torch.float32)
            data_loader = get_data_loader_BERT(self.config, dataset) # shuffle=False
            lbs = []
            for step, (instance, labels, ind) in enumerate(data_loader):
                for k in instance.keys():
...
#         neg = 0.6*t2 + 1 - self.m
    ...
    # def contrastive_loss_des(self, des, x, labels,temp = 1.0, is_memory=False):
    #     """
    #     Computes the supervised contrastive loss between pairs (x, des) based on their labels.
    ...
    #     # Compute similarity matrix (batch_size x batch_size)
    #     sim_matrix = torch.matmul(x, des.T) / temp  # 0.07 is the temperature scaling factor τ
    ...
    #     # Compute the sum of the numerator (positive pairs) and the denominator (all pairs)
    #     numerator = (positive_mask * torch.exp(sim_matrix)).sum(dim=1)  # Sum of all positive pairs

    #     denominator = numerator + (negative_mask * torch.exp(sim_matrix)).sum(dim=1)
...
# def contrastive_loss(self, x, des, labels, is_memory=False):
# give me this contrastive loss betwwen x and des where pos is the pair that has the same labels in labels
# for example labels: [0,2,2,3] then x[1], des[1] ; x[1], des[2] ; x[2], des[1] ; x[2], des[2] will be positive pair 
