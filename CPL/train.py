...
from losses import OTLoss
from sampler import data_sampler_CFRL
from data_loader import get_data_loader_BERT
from utils import Moment, gen_data
from encoder import EncodingModel, EncodingModel_LLM2Vec
import wandb
...
class Manager(object):
    ...
    def select_memory(self, encoder, dataset):
        ...
        features = features.to(torch.float32).detach().cpu().numpy()
    ...
    def train_model(self, encoder, teacher, training_data, is_memory=False):
        ...
        for i in range(epoch):
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                hidden2 = teacher(instance['input'])
    ...
    def eval_encoder_proto(self, encoder, seen_proto, seen_relid, test_data):
        ...
        for batch_num, (instance, label, _) in enumerate(test_loader):
            ...
            corrects += correct
            total += batch_size
            wandb.log({'batch_num': batch_num, 'acc': 100 * acc, 'total acc': 100 * (corrects / total)})
    ...
    def train(self):
        ...
        # self.teacher_tokenizer = AutoTokenizer.from_pretrained(self.config.llm_path, trust_remote_code=True)
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            self.train_model(encoder, teacher, training_data_initialize)
            ...
            if step > 0:
                ...
                self.train_model(encoder, teacher, memory_data_initialize, is_memory=True)
            ...
            wandb.log({'step': step, 'cur_acc': ac1, 'his_acc': ac2})
...
if __name__ == '__main__':
    ...
    for i in range(len(ave)):
        wandb.log({f'his_acc_mean': ave[i]})
    # with open(f'./result/{config.task_name}_{config.num_k}-shot.txt', 'w') as f:
